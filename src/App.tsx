...
import { Trophy, Star, X, Settings as SettingsIcon, RefreshCw, User, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function App() {
  ...
  // Lưu tên học sinh tự động
  useEffect(() => {
    localStorage.setItem('studentName', studentName);
  }, [studentName]);

  // Kiểm tra phiên bản mới (OTA)
...
// Xử lý cập nhật OTA
  const handleUpdate = async () => {
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        // 1. Dùng URL gốc thay vì fallback
        const downloadUrl = updateUrl || `https://github.com/TontonYuta/math6/releases/download/v${newVersion}/update.zip`;
        
        setUpdateProgressText('Đang tải bản cập nhật...');
        
        // 2. Tải bản cập nhật về điện thoại
        const version = await CapacitorUpdater.download({
          url: downloadUrl,
          version: newVersion, // Đảm bảo truyền đúng số phiên bản
        });
        
        setUpdateProgressText('Đang cài đặt...');
        
        // 3. Khởi động lại App và áp dụng bản mới
  ...
  // Tải dữ liệu tiến trình và màu sắc
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-black dark:via-[#0a0f1d] dark:to-black font-sans text-slate-900 dark:text-slate-100 selection:bg-indigo-200 dark:selection:bg-indigo-900 transition-colors duration-500 scroll-smooth antialiased">
      ...
      <AnimatePresence>
        {showUpdateBanner && (
          ...
          >
            <span className="font-medium text-sm tracking-wide">{isUpdating ? updateProgressText : updateMessage}</span>
            <button 
      ...
      <main className="container mx-auto px-4 py-8 max-w-3xl">
        <AnimatePresence mode="wait">
          {appState === 'home' && (
            ...
            >
              {/* Header: Khung nhập tên và nút Cài đặt */}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 uppercase tracking-wider">Thành tích</h2>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-medium">
                          Đã mở khóa: <span className="text-indigo-600 dark:text-indigo-400 font-bold">{progress.unlockedAchievements.length}</span>/{achievements.length}
                        </p>
                      </div>
                    </div>
                          ...
                          {achievements.map(achievement => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <h3 className="text-xs font-bold text-slate-800 dark:text-slate-200 leading-tight mb-1">{achievement.title}</h3>
          ...
          >
            <div className="bg-indigo-600/95 backdrop-blur-md text-white p-6 rounded-[2rem] shadow-2xl flex items-center gap-4 relative overflow-hidden border border-indigo-500/50">
              <div className="absolute top-0 right-0 p-3">
                <button onClick={() => setNewAchievement(null)} className="text-white/50 hover:text-white transition-colors active:scale-90">
                  <X size={20} />
                </button>
              </div>
